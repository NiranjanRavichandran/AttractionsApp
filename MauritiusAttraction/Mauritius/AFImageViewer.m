...
{
    //NSLog(@"Current offset %f", _imageScrollView.contentOffset.x);
...
-(void)setCurrentPage:(BOOL)isForward{
    CGFloat pageWidth = self.imageScrollView.frame.size.width;
    if (isForward){
        [self.imageScrollView setContentOffset:CGPointMake(pageWidth - self.imageScrollView.contentOffset.x, 0) animated:YES];
    }else {
        [self.imageScrollView setContentOffset:CGPointMake(pageWidth - self.imageScrollView.contentOffset.x, 0) animated:YES];
    }
}
